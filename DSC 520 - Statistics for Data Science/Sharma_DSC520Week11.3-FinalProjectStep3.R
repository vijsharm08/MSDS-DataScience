---
  title: "Sharma_DSC520Week11.3FinalStep3"
output: pdf_document
date: "2025-02-28"
---
...
RemCustomer <- subset(churn, select = -customerID)
head(RemCustomer)	
...
p9 <-  ggplot(datrecode, aes(x = Contract, fill = Contract)) +
  ...
  geom_text(aes(y = ..count.. -400, 
                label = paste0(round(prop.table(..count..),
                                     1)*100, "%")),stat = "count")+
  labs(title = "Distribution of Contract Type",
       x = "Contract Type",
       y = "Count") +
  scale_fill_brewer(palette = "Set2") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Create a bar chart for visualization
ggplot(data.frame(group = c("Senior Citizen", "Non-Senior Citizen"), 
                  avg_charges = c(avg_total_charges_senior, 
                                  avg_total_charges_non_senior)), 
       aes(x = group, y = avg_charges, fill = group)) + 
  geom_bar(stat = "identity") + 
